...
import wms.domain.category.dao.ContractTypeRepo;
import wms.domain.category.dao.ProductCategoryRepo;
...
import wms.domain.sellin.dao.ProductPriceRepo;
import wms.domain.sellin.entity.ProductPrice;
import wms.domain.sellout.dao.ProductSalePriceRepo;
...
public class ProductServiceImpl extends BaseService implements IProductService {
    ...
    public List<ProductEntity> createProductFromExcel(MultipartFile file) throws IOException, CustomException {
        ...
        if (fileName.equals("")) {
            throw caughtException(ResponseCode.USER_ACTION_FAILED.getCode(), "Hãy tải file lên hệ thống!");
        }
        String fileExt = "";
        ...
        if (!fileExt.contains("xls")) {
            throw caughtException(ResponseCode.FORMAT.getCode(), "Wrong file type. Only support .xls and .xlsx file extensions");
        }
...
//        FileInputStream fis = new FileInputStream(path.replace(".", "data/file/") + fileName);
    ...
    public ProductEntity createProduct(ProductDTO productDTO) throws CustomException {
        if (productRepository.getProductBySku(productDTO.getSku()) != null) {
            throw caughtException(ResponseCode.ALREADY_EXIST.getCode(), "Exist product with same sku, can't create");
        }
        ...
        if (unit== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product with no specific unit, can't create");
        }
        if (category == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product with no specific category, can't create");
        }
    ...
    public ProductEntity updateProduct(ProductDTO productDTO, long id) throws CustomException {
        ...
        if (productBySku != null && productBySku.getId() != id) {
            throw caughtException(ResponseCode.ALREADY_EXIST.getCode(), "Exist product with same sku, can't update");
        }
        ProductUnit unit = productUnitRepo.getProductUnitById(productDTO.getUnitId());
        ProductCategory category = productCategoryRepo.getProductCategoryById(productDTO.getCategoryId());

        if (unit== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product with no specific unit, can't update");
        }
        if (category == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product with no specific category, can't update");
        }
...
//            }
            if (product== null) {
                throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product price with no specific, can't set");
            }
...
//                    .contractType(contractType)
    ...
    public ProductPrice updateSellinPrice(ProductPriceDTO productPriceDTO) throws CustomException {
        ...
        if (productPrice == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "None existed product, can't update");
        }
    ...
    public void setSalePrice(List<ProductDiscountDTO> productDiscountDTO) throws CustomException {
        for (ProductDiscountDTO discountDTO : productDiscountDTO) {
            ...
            if (contractType == null) {
                throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product price with no specific contract type, can't set");
            }
            if (product == null) {
                throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product price with no specific, can't set");
            }

...
import wms.domain.category.dao.CustomerTypeRepo;
import wms.domain.category.entity.ContractType;
import wms.domain.category.entity.Customer;
import wms.domain.category.entity.CustomerType;
...
public class CustomerServiceImpl extends BaseService implements ICustomerService {
    ...
    public void createCustomerFromFile(MultipartFile file, JwtAuthenticationToken token) throws IOException, CustomException {
        ...
        if (createdBy == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown staff create this customer, can't create");
        }
    ...
    public Customer createNewCustomer(CustomerDTO customer, JwtAuthenticationToken token) throws CustomException {
        ...
        if (customerType== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Customer with no specific type, can't create");
        }
        if (contractType == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Customer with no specific contract, can't create");
        }
        if (createdBy == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown staff create this customer, can't create");
        }
        if (customer.getLongitude() == "" || customer.getLatitude() == "") {
            throw caughtException(ResponseCode.FORMAT.getCode(), "Location didn't validate, can't create");
        }
        List<Facility> facilities = facilityRepo.getAllFacility();
        int bestFacilityIndex = 0;
        double bestDistance = Double.POSITIVE_INFINITY;
    ...
    public Customer updateCustomerInfo(CustomerUpdateDTO customerDTO, long id) throws CustomException {
        ...
        if (customerType== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Customer with no specific type, can't update");
        }
        if (contractType == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Customer with no specific contract, can't update");
        }

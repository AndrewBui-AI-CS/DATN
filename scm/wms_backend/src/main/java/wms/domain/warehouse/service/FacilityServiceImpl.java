...
import wms.domain.category.dao.CustomerRepo;
import wms.domain.category.dao.ProductRepo;
import wms.domain.category.dao.ProductUnitRepo;
import wms.domain.category.entity.Customer;
import wms.domain.category.entity.ProductEntity;
import wms.domain.sellin.dao.PurchaseOrderItemRepo;
import wms.domain.sellin.dao.PurchaseOrderRepo;
import wms.domain.sellin.dao.ReceiptBillItemRepo;
import wms.domain.sellin.dao.ReceiptBillRepo;
import wms.domain.sellin.entity.PurchaseOrder;
import wms.domain.sellin.entity.PurchaseOrderItem;
import wms.domain.sellin.entity.ReceiptBill;
import wms.domain.sellin.entity.ReceiptBillItem;
import wms.domain.sellout.dao.DeliveryBillItemRepo;
import wms.domain.sellout.dao.DeliveryBillRepo;
import wms.domain.sellout.dao.SaleOrderItemRepo;
import wms.domain.sellout.dao.SaleOrderRepo;
...
import wms.domain.warehouse.dao.ProductFacilityRepo;
import wms.domain.warehouse.dto.*;
import wms.domain.warehouse.entity.Facility;
import wms.domain.warehouse.entity.InventoryItem;
import wms.domain.warehouse.entity.ProductFacility;
import wms.common.ReturnPaginationDTO;
import wms.exception.CustomException;
import wms.common.BaseService;
import wms.domain.utils.files.IFileService;
import wms.domain.sellin.service.IPurchaseOrderService;
import wms.domain.sellout.service.ISaleOrderService;
...
public class FacilityServiceImpl extends BaseService implements IFacilityService {
    ...
    public void createFacilityFromFile(MultipartFile file, JwtAuthenticationToken token) throws IOException, CustomException {
        ...
        if (createdBy == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown staff create this facility, can't create");
        }
    ...
    private void saveFacilities(Row row, UserRegister createdBy) throws CustomException {
        ...
        if (manager == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown manager manage this facility, can't create");
        }
    ...
    public Facility updateFacility(FacilityUpdateDTO facilityDTO, Long id) throws CustomException {
        ...
        if (facilityToUpdate == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Facility does not exist, can't update");
        }
        UserRegister manager = userRepo.getUserByUserLoginId(facilityDTO.getManagedBy());
        if (manager == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Manager unknown");
        }
    ...
    public void importToFacilityWithOrder(ImportToFacilityDTO importToFacilityDTO) throws CustomException {
        ...
        if (order == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Can't find referenced order, can't import");
        }
        if (!canImportToFacility(order, importToFacilityDTO)) {
            throw caughtException(ResponseCode.USER_ACTION_FAILED.getCode(), "Exceed warehouse quantity limit, can't import");
        }
        if (isExceedPurchaseOrderQuantity(order, importToFacilityDTO)) {
            throw caughtException(ResponseCode.USER_ACTION_FAILED.getCode(), "Import with quantity more than current order needs, can't import");
        }
        ...
        for (ImportItemDTO importItem : importToFacilityDTO.getImportItems()) {
            ...
            if (product == null) {
                ...
                throw caughtException(ResponseCode.NON_EXIST.getCode(), "Product not found");
            }
    ...
    public void exportFromFacility(ExportFromFacilityDTO exportFromFacilityDTO) throws CustomException {
        ...
        if (order == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Can't find referenced order, can't export");
        }
        if (!canExportFromFacility(order, exportFromFacilityDTO)) {
            throw caughtException(ResponseCode.USER_ACTION_FAILED.getCode(), "Exceed warehouse quantity limit, can't export");
        }
        if (isBeyondSaleOrderQty(order, exportFromFacilityDTO)) {
            throw caughtException(ResponseCode.USER_ACTION_FAILED.getCode(), "Export with quantity more than current order needs, can't export");
        }
    ...
    public void assignStaff(String staffCode, String facilityCode) throws CustomException {
        ...
        if (userLogin == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown staff to assign this facility, can't create");
        }

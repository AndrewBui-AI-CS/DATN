...
import wms.domain.sellout.dto.sale_order.SaleOrderItemDTO;
import wms.domain.sellout.dto.sale_order.UpdateSaleOrderDTO;
import wms.exception.CustomException;
import wms.common.BaseService;
import wms.domain.utils.files.ExportPDFService;
...
public class SaleOrderServiceImpl extends BaseService implements ISaleOrderService {
    ...
    public SaleOrder createOrder(SaleOrderDTO saleOrderDTO, JwtAuthenticationToken token) throws CustomException {
        ...
        if (boughtBy== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Order belongs to no customer, can't create");
        }
        if (createdBy == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Order created by unknown person, can't create");
        }
    ...
    public SaleOrder updateOrder(UpdateSaleOrderDTO updateSaleOrderDTO) throws CustomException {
        ...
        if (currOrder == null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Unknown order, can't update");
        }
        ...
        if (boughtBy== null) {
            throw caughtException(ResponseCode.NON_EXIST.getCode(), "Order belongs to no customer, can't update");
        }
        int seq = 0;
